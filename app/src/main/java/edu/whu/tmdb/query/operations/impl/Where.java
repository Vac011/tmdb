...
public class Where {
    ...
    public SelectResult orExpression(OrExpression expression,SelectResult selectResult) throws TMDBException, IOException {
        Expression left = expression.getLeftExpression();
        Expression right = expression.getRightExpression();
        SelectResult result1 = new SelectResult(selectResult.getTpl(), selectResult.getClassName(), selectResult.getAttrname(), selectResult.getAlias(), selectResult.getAttrid(), selectResult.getType());
        SelectResult result2 = new SelectResult(selectResult.getTpl(), selectResult.getClassName(), selectResult.getAttrname(), selectResult.getAlias(), selectResult.getAttrid(), selectResult.getType());
        SelectResult selectResult1 = execute(left, result1);
        SelectResult selectResult2 = execute(right, result2);
        HashSet<Tuple> selectResultSet1 = getTupleSet(selectResult1);
        HashSet<Tuple> selectResultSet2 = getTupleSet(selectResult2);
        HashSet<Tuple> union = new HashSet<>();
        //将第一个条件的所有tuple加入overlap中
        for(Tuple tuple:selectResultSet1){
            union.add(tuple);
        }
        //将第二个条件中不在第一个的tuple加入overlap中
        for(Tuple tuple:selectResultSet2){
            if(!selectResultSet1.contains(tuple)) union.add(tuple);
        }
