...
public class SelectImpl implements edu.whu.tmdb.query.operations.Select {
    ...
    private SelectResult limit(int limit,SelectResult selectResult) {
        TupleList oldtpl = selectResult.getTpl(); 
        if(limit >= oldtpl.tuplenum)
        {
            return selectResult;
        }
        //将原tpl中的前limit个tuple加入到新tpl中
        TupleList newtpl = new TupleList();
        for(int i=0; i<limit; i++)
        {
            newtpl.addTuple(oldtpl.tuplelist.get(i));
        }
        selectResult.setTpl(newtpl);
        return selectResult;
    }
...
//            这里是针对selectItem是表达式的判定，a或者a+2或者a+b*c都会被认定为表达式
    ...
    private void projectSelectExpression(SelectItem item, SelectResult entireResult,
        ...
        // 2.alias赋值
        String alias = selectItem.getAlias() != null ? selectItem.getAlias().toString() : null;
        // 处理别名，只保留别名部分而不包含关键字 AS
        if (alias != null && alias.toUpperCase().startsWith(" AS ")) {
            alias = alias.substring(3).trim(); // 去掉关键字 AS，并去除前后空格
        }
        // 3.resTupleList赋值
        ArrayList<String> TableColumn = new ArrayList<>();          // 含有两个元素的列表，结构为[tableName, columnName]
        attributeParser(attrName, TableColumn);
        String tableName = TableColumn.get(0); // 获取表
        String columnName = TableColumn.get(1); // 获取属性名
        ...
        int indexInEntireResult = getIndexInEntireResult(entireResult, tableName, columnName);
        if (indexInEntireResult == -1) {
            throw new TMDBException(/* handle exception */); // 如果未找到属性在完全查询结果中的位置，抛出异常
        }
        //  将值列表添加到投影结果中
        for (int i = 0; i < resTupleList.tuplelist.size(); i++) {
            Tuple tuple = resTupleList.tuplelist.get(i);
            tuple.tuple[indexInResult] = dataList.get(i); // 将对应位置的值赋给投影结果的元组
            tuple.classId = entireResult.getTpl().tuplelist.get(i).classId;
            tuple.tupleId = entireResult.getTpl().tuplelist.get(i).tupleId; // 设置对应的元组标识
        }
        // 4.剩余属性赋值, 更新投影结果的属性信息
        projectResult.getClassName()[indexInResult] = entireResult.getClassName()[indexInEntireResult]; // 设置表名
        projectResult.getAttrname()[indexInResult] = (alias != null) ? alias : attrName; // 设置属性名
        projectResult.getAlias()[indexInResult] = attrName; // 将别名设置为原属性名
        projectResult.getType()[indexInResult] = entireResult.getType()[indexInEntireResult]; // 设置属性类型
        projectResult.getAttrid()[indexInResult] = entireResult.getAttrid()[indexInEntireResult]; // 设置属性在结果中的下标
     ...
     */
    public void attributeParser(String expression, ArrayList<String> TableColumn) {
        String attr;
...
// 这里有问题，应该将表头信息同样合并
        String[] classnames = Stream.concat(Arrays.stream(selectResult1.getClassName()), Arrays.stream(selectResult2.getClassName())).toArray(String[]::new);
        // String[] attrnames = Stream.concat(Arrays.stream(selectResult1.getAttrname()), Arrays.stream(selectResult2.getAttrname())).toArray(String[]::new);
        String[] alias = Stream.concat(Arrays.stream(selectResult1.getAlias()), Arrays.stream(selectResult2.getAlias())).toArray(String[]::new);
        int[] attrids = IntStream.concat(Arrays.stream(selectResult1.getAttrid()), Arrays.stream(selectResult2.getAttrid())).toArray();
        String[] types = Stream.concat(Arrays.stream(selectResult1.getType()), Arrays.stream(selectResult2.getType())).toArray(String[]::new);
        selectResult1.setClassName(classnames);
        // selectResult1.setAttrname(attrnames);
        selectResult1.setAlias(alias);
        selectResult1.setAttrid(attrids);
        selectResult1.setType(types);
