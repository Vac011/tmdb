...
public class MemConnect {
    ...
    /**
     * 给定id, 获取表在classTable中的表名
     * @param tableName id
     * @return 给定id所对应的表名(类名)
    ...
    public int getClassAttrnum(int classId) throws TMDBException {
        ...
        // 不存在时抛出异常
        throw new TMDBException(ErrorList.CLASS_ID_DOES_NOT_EXIST, Integer.toString(classId));
    }
    ...
    public int[] getAttridList(int classId, List<String> columns) throws TMDBException {
        boolean flag = false;
        int n = columns.size();
        int[] attrIdList = new int[n];
        for(ClassTableItem item : getClassTableList()) {
            if(item.classid == classId && columns.contains(item.attrname)) {
                flag = true;
                attrIdList[columns.indexOf(item.attrname)] = item.attrid;
            }
        }
        if(flag){
            return attrIdList;
